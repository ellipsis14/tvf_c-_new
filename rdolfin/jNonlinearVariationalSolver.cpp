...
#include "jNonlinearVariationalSolver.h"

using namespace dolfin;

//-----------------------------------------------------------------------------
jNonlinearVariationalSolver::jNonlinearVariationalSolver(std::shared_ptr<NonlinearVariationalProblem> problem)
  : _problem(problem)
{
  // Set parameters
...
//-----------------------------------------------------------------------------
//jMODS:
std::tuple<std::size_t, bool, double, double> jNonlinearVariationalSolver::solve()
// std::pair<std::size_t, bool> jNonlinearVariationalSolver::solve()
{
  ...
  //jMODS:
  std::tuple<std::size_t, bool, double, double> jret;
  if (std::string(parameters["nonlinear_solver"]) == "newton")
  {
    ...
    if (!newton_solver)
    {
      ...
      newton_solver = std::make_shared<jNewtonSolver>(comm);
    }
...
////////////////////////////////////////////////////////////////////////    
    jret = newton_solver->solve(*nonlinear_problem, *u->vector());
...
//-----------------------------------------------------------------------------
jNonlinearVariationalSolver::NonlinearDiscreteProblem::
NonlinearDiscreteProblem(std::shared_ptr<const NonlinearVariationalProblem> problem,
                         std::shared_ptr<const jNonlinearVariationalSolver> solver)
  : _problem(problem), _solver(solver)
{
  // Do nothing
}
//-----------------------------------------------------------------------------
jNonlinearVariationalSolver::NonlinearDiscreteProblem::~NonlinearDiscreteProblem()
{
  // Do nothing
}
//-----------------------------------------------------------------------------
void jNonlinearVariationalSolver::
...
//-----------------------------------------------------------------------------
void
jNonlinearVariationalSolver::NonlinearDiscreteProblem::J(GenericMatrix& A,
